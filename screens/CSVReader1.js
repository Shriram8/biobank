...
//variables declaring and parsing values to table3
 ...
 for(var i = 1; i<data.length-1; i++){
   ...
   Record_ID = temp[0],
    ...
    Age =  parseInt(temp[2]),
   Date_of_Birth =  new Date(temp[3]).toISOString().slice(0, 10) || 0,
   Sex =  parseInt(temp[4]),
   ...
   BMI_range = temp[9]
    Date_of_positive_SARS_COV2_PCR =  new Date(temp[10]).toISOString().slice(0, 10) || 0 ,
    Date_of_first_collection = new Date(temp[11]).toISOString().slice(0, 10) || 0
   Past_medical_history = temp[12] ,
   Employed =  parseInt(temp[13]),
   Tobacco_use = parseInt(temp[14]),
   packs_per_day =  parseInt(temp[15]),
